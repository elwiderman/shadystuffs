...
class Xoo_Sl_Admin_Settings{
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'xoo-sl-admin-js', XOO_SL_URL.'/admin/assets/js/xoo-sl-admin-js.js', array( 'jquery' ), XOO_SL_VERSION, true );
	...
	public function user_page_columns_output( $val, $column_name, $user_id ){
		if( $column_name === "xoo_sl_login_type" ){
			...
			if( $social_type ){
				ob_start();
				include XOO_SL_PATH.'/assets/icons/'.$social_type.'.svg';
				$val = ob_get_clean();
			}
			else{
