...
class Xoo_Sl_Frontend{
	...
	public function enqueue_scripts(){
		...
		if( $this->appleSettings['gl-enable'] === "yes" ){
			$locale = $this->appleSettings['btn-locale'] ? esc_html( $this->appleSettings['btn-locale'] ) : get_locale();
			wp_enqueue_script( 'xoo-sl-apple-sdk', 'https://appleid.cdn-apple.com/appleauth/static/jsapi/appleid/1/'.$locale.'/appleid.auth.js' );
		}
		...
		wp_localize_script( 'xoo-sl-js', 'xoo_sl_localize', array(
			...
			'apple' 	=> array(
				'enable' 		=> $this->appleSettings['gl-enable'],
				'initArgs' 		=> array(
					'clientId' 		=> esc_html( $this->appleSettings['gl-clientid'] ),
					'scope' 		=> 'name email',
					'redirectURI' 	=> get_site_url(),
					'state' 		=> bin2hex(random_bytes(5)),
					'usePopup' 		=> true
				),
				'client_id' 	=> $this->appleSettings['gl-clientid']
			),
