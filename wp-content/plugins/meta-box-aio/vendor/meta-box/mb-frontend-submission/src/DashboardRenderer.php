...
use MBFS\Block\Attributes;
use MBFS\Elementor\Meta;
use MBFS\Bricks\BricksMeta;
...
class DashboardRenderer {
	...
	public function render( $atts ) {
		...
		if ( empty( $this->edit_page_atts ) ) {
			...
			return '<div class="rwmb-error">' . sprintf( __( 'Something is not right with the shortcode on page ID: %d', 'mb-frontend-submission' ), $edit_page_id ) . '</div>';
		}
	...
	private function get_edit_page_attrs( int $edit_page_id ): array {
		...
		if ( empty( $attributes ) ) {
			$attributes = ( new Attributes() )->get_attributes( $edit_page_id );
		}

		// Get attributes Elementor widget.
		if ( empty( $attributes ) ) {
			$attributes = ( new Meta() )->get_attributes( $edit_page_id );
		}

		// Get attributes Bicks widget.
		if ( empty( $attributes ) ) {
			$attributes = ( new BricksMeta() )->get_attributes( $edit_page_id );
		}
	...
	private function get_edit_page_attrs_from_shortcode( int $edit_page_id ): array {
		...
		foreach ( $shortcode_atts as $attribute ) {
			...
			if ( in_array( $attribute[0], [ 'id', 'post_type' ], true ) ) {
